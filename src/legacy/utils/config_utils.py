...
class BaseConfig:
    ...
    use_llm_rerank: bool = field(
        default=False,
        metadata={"help": "Whether to enable LLM-based reranking (DSPyFilter)."}
    )
    ...
    topic_extraction_mode: Literal["default", "fine_grained"] = field(
        default="default",
        metadata={"help": "Topic extraction mode: 'default' for standard extraction, 'fine_grained' for detailed extraction with more context."}
    )

    # Topic extraction LLM specific attributes (optional, for using a different model)
    topic_extraction_llm_name: str = field(
        default=None,
        metadata={"help": "LLM model name for topic extraction. If None, uses the main llm_name."}
    )
    topic_extraction_azure_endpoint: str = field(
        default=None,
        metadata={"help": "Azure endpoint for topic extraction LLM. If None, uses the main azure_endpoint."}
    )
    topic_extraction_api_key: str = field(
        default=None,
        metadata={"help": "API key for topic extraction LLM. If None, uses the main API key."}
    )
    topic_extraction_api_version: str = field(
        default="2024-02-15-preview",
        metadata={"help": "API version for topic extraction LLM."}
    )
    ...
    qa_prompt_name: Optional[str] = field(
        default=None,
        metadata={"help": "Override QA prompt template name if provided."}
    )
    ...
    dense_rerank_topk: int = field(
        default=200,
        metadata={"help": "Number of top passages (after PPR) to re-rank with dense retrieval."}
    )
    dense_fuse_alpha: float = field(
        default=0.7,
        metadata={"help": "Fusion weight between dense and PPR scores. 0=PPR only, 1=Dense only."}
    )
    dense_fuse_gamma: Optional[float] = field(
        default=None,
        metadata={"help": "Optional fusion weight between PPR and cosine similarity scores. If None, use dense_fuse_alpha logic."}
    )
    dense_fuse_rrf: bool = field(
        default=False,
        metadata={"help": "If True, use RRF rank fusion for ppr/dense in Top-K rerank."}
    )
    dense_fuse_rrf_k: float = field(
        default=60.0,
        metadata={"help": "RRF k parameter (larger -> flatter)."}
    )

    # Entity reward hyperparameters (optional overrides)
    entity_alpha: Optional[float] = field(
        default=None,
        metadata={"help": "Override entity reward alpha (None = use dataset defaults)."}
    )
    entity_beta: Optional[float] = field(
        default=None,
        metadata={"help": "Override entity reward beta (None = use dataset defaults)."}
    )
