...
class PromptTemplateManager:
    ...
    def _load_templates(self) -> None:
        ...
        for filename in os.listdir(self.templates_dir):
            if filename.endswith(".py") and filename != "__init__.py":
                ...
                try:
                    try:
                        module_name = f"src.cogitorag.prompts.templates.{script_name}"
                        module = importlib.import_module(module_name)
                    except ModuleNotFoundError:
                        try:
                            module_name = f"src.legacy.prompts.templates.{script_name}"
                            module = importlib.import_module(module_name)
                        except ModuleNotFoundError:
                            module_name = f".prompts.templates.{script_name}"
                            module = importlib.import_module(module_name, 'legacy')
    ...
    def render(self, name: str, **kwargs) -> Union[str, List[Dict[str, Any]]]:
        ...
        # print(f"[dev] [PromptTemplateManager] [render] template: \n{template}") # dev add
