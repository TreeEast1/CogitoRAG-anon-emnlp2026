...
class CacheOpenAI(BaseLLM):
    ...
    @classmethod
    def from_custom_config(cls, cache_dir: str, llm_name: str, azure_endpoint: str, api_key: str,
                           api_version: str = "2024-02-15-preview", **kwargs) -> "CacheOpenAI":
        """
        Create a CacheOpenAI instance with custom configuration.
        Used for topic extraction with a different model.
        """
        return cls(
            cache_dir=cache_dir,
            global_config=None,
    ...
    def __init__(self, cache_dir, global_config, cache_filename: str = None,
                 ...
                 llm_name: str = None, azure_endpoint: str = None,
                 api_key: str = None, api_version: str = "2024-02-15-preview",
                 **kwargs) -> None:
        ...
        # Support custom config (for topic extraction with different model)
        ...
        if self._azure_endpoint is None:
            ...
            # Use custom api_key if provided, otherwise use default
            if self._api_key is not None:
                ...
                # Default API key for Sweden Central endpoint
                self.openai_client = AzureOpenAI(
    ...
    def _init_llm_config(self) -> None:
        if self.global_config is not None:
            ...
            # Custom config mode - create minimal config
            config_dict = {
                'llm_name': self.llm_name,
                'llm_base_url': self.llm_base_url,
                'generate_params': {
                    "model": self.llm_name,
                    "max_completion_tokens": 2000,  # Larger for topic extraction
                    "n": 1,
                    "seed": 0,
                    "temperature": 0.0,
                }
