...
class DSPyFilter:
    def __init__(self, parent):
        ...
        Parameters:
        parent : An object that provides the global configuration and the LLM model required for inference.
        ...
        """
        dspy_file_path = parent.global_config.rerank_dspy_file_path
        ...
        self.llm_infer_fn = parent.llm_model.infer
        self.model_name = parent.global_config.llm_name
    ...
    def llm_call(self, question, fact_before_filter):
        ...
        if isinstance(response, tuple):
            if len(response) > 1:
                self.last_metadata = response[1] or {}
            if len(response) > 2:
                self.last_cache_hit = bool(response[2])
            return response[0]
